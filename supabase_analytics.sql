...
BEGIN
  ...
  SET 
    left_score = left_score + (NEW.axis_scores->>Object.keys(NEW.axis_scores)[0])::INTEGER,
    right_score = right_score + (NEW.axis_scores->>Object.keys(NEW.axis_scores)[1])::INTEGER,
    total_answers = total_answers + (NEW.axis_scores->>'total')::INTEGER,
    last_updated = NOW()
  WHERE axis = Object.keys(NEW.axis_scores)[0];
